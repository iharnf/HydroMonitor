...
```yaml
ph_cal_midpoint: "2610"       # mV при pH 7.0
ph_cal_slope: "156.67"        # mV/pH
```

`slope` меняется только через прошивку. `midpoint` можно откалибровать кнопкой **Button B**:

1. Опусти электрод в буфер pH 7.0, дождись стабилизации (30-60 с)
2. Нажми **Button B** — текущее mV сохранится как новый midpoint
3. На экране загорится **"CAL pH7 OK"** на 3 секунды
4. Значение хранится в flash-памяти ESP32, не сбрасывается при перезагрузке

Текущий midpoint виден в Home Assistant как сенсор `pH Cal Midpoint`.
